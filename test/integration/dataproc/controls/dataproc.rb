...
dataproc_subnet_name   = attribute('dataproc_subnet_name')
dataproc_subnet_region = attribute('dataproc_subnet_region')

control 'svpc-dataproc-access' do
  title "Dataproc shared VPC access"
  ...
  describe command("gcloud beta compute networks subnets get-iam-policy #{dataproc_subnet_name} --region #{dataproc_subnet_region} --project #{host_project_id} --format=json") do
    its('exit_status') { should eq 0 }
    its('stderr') { should eq '' }
