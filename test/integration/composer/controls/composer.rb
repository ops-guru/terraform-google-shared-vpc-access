...
composer_subnet_name   = attribute('composer_subnet_name')
composer_subnet_region = attribute('composer_subnet_region')

control 'svpc-composer-access' do
  title "Composer shared VPC access"
  ...
  describe command("gcloud beta compute networks subnets get-iam-policy #{composer_subnet_name} --region #{composer_subnet_region} --project #{host_project_id} --format=json") do
    its('exit_status') { should eq 0 }
    its('stderr') { should eq '' }
